...
####################################
# Shared feature extractor (using conv layers from a simple CNN)
...
def incremental_learning_moe(model, train_dataset, train_target, test_dataset, test_target,
    ...
    for task in range(num_tasks):
        ...
        # For the first task, use 100 epochs; otherwise, use the provided num_epochs.
...
####################################
# Hyperparameters
num_tasks = 4  # For example, if GTSRB has 43 classes, 6 tasks with roughly equal classes per task.
nclasses = len(np.unique(train_target))
classes_per_task = math.ceil(nclasses / num_tasks)
batch_size = 64
lr = 1e-3
num_epochs = 40     # Default epochs per task (except the first task, which uses 100)
